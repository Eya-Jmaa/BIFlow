...
import { Cell, Row, Table } from "@/components/ui/data";
        ...
        <div className="flex shrink-0 items-center gap-1">
          {onDrill && (
            <button
              type="button"
              onClick={onDrill}
              className="grid size-7 place-items-center rounded-md text-ink-faint transition-colors hover:bg-surface-sunken hover:text-ink"
          ...
          <div className="flex rounded-lg bg-surface-sunken p-0.5">
            <Toggle active={!showTable} onClick={() => setShowTable(false)}>
              Chart
            </Toggle>
            <Toggle active={showTable} onClick={() => setShowTable(true)}>
              Table
            </Toggle>
          </div>
        </div>
      ...
      >
        {rows.map((row) => (
          <Row key={row.key}>
            ...
            <Cell numeric>{formatValue(row.value, format, { unit })}</Cell>
          </Row>
        ))}
      </Table>
    </div>
        ...
        {rows.length === 0 ? (
          ...
          <div className="max-h-80 overflow-auto">
            ...
            >
              {rows.map((row) => (
                <Row key={row.name}>
                  <Cell className="font-medium text-ink">{row.name}</Cell>
                  <Cell numeric>{formatValue(row.value, {}, { unit: row.unit })}</Cell>
                  <Cell className="hidden font-mono text-[0.625rem] text-ink-faint md:table-cell">
                    {row.formula}
                  </Cell>
                </Row>
              ))}
    ...
    return (
      <Card className="h-full">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <Badge tone={flagged.length ? "warning" : "success"} dot>
          ...
          <div className="max-h-60 overflow-auto">
            ...
            >
              {flagged.slice(0, 20).map((item, index) => (
                <Row key={`${item.metric}-${index}`}>
                  <Cell className="font-medium text-ink">{item.metric}</Cell>
                  <Cell>{item.period ? shortPeriod(item.period) : "—"}</Cell>
                  <Cell numeric>{formatValue(item.value, {}, { unit: item.unit })}</Cell>
